      ...
      <div class="drawer-header">
        <div class="drawer-title">
          ...
          <div>
            ...
            <p>{{ isEdit ? 'Chỉnh sửa thông tin sản phẩm' : 'Tạo sản phẩm mới trong hệ thống' }}</p>
          </div>
        </div>
          ...
          <div class="form-content">
              ...
              <div class="section-body">
                <div class="row g-4">
                  <div class="col-12">
                    ...
                    <input v-model.trim="formData.name" class="form-control" placeholder="Ví dụ: Dior Sauvage Eau De Parfum">
                  </div>

                  <div class="col-lg-4">
                    <label class="form-label">Thương hiệu <span class="text-danger">*</span></label>
                    <select v-model.number="formData.brandId" class="form-select">
                  ...
                  <div class="col-lg-4">
                    ...
                    <select v-model.number="formData.categoryId" class="form-select">
                  ...
                  <div class="col-lg-4">
                    <label class="form-label">Nồng độ <span class="text-danger">*</span></label>
                    <select v-model.number="formData.concentrationId" class="form-select">
                      <option :value="0">Chọn nồng độ</option>
                      <option v-for="item in concentrationList" :key="item.id" :value="item.id">
                  ...
                  <div class="col-lg-4">
                    ...
                    <select v-model.number="formData.gender" class="form-select">
                  ...
                  <div class="col-lg-4 d-flex align-items-end mb-2">
                    <div class="form-check form-switch fs-6 mb-2">
                      <input id="isNiche" v-model="formData.isNiche" class="form-check-input" type="checkbox">
                      <label for="isNiche" class="form-check-label ms-2">Nước hoa Niche</label>
                    </div>
                  </div>

                  <div class="col-12">
                    <label class="form-label">Mô tả sản phẩm</label>
                    <textarea v-model="formData.description" class="form-control" rows="4" placeholder="Nhập mô tả chi tiết về sản phẩm..."></textarea>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section class="content-card">
              <div class="section-header">
                ...
                <div class="section-badge">{{ formData.fragranceFamilyIds.length }} nhóm hương</div>
              </div>
              <div class="section-body">
                <div class="row g-3">
                  <div class="col-xl-3 col-lg-4 col-md-6" v-for="item in fragranceFamilyList" :key="item.id">
                    <label class="fragrance-card" :class="{ active: formData.fragranceFamilyIds.includes(item.id) }">
                      <input type="checkbox" :value="item.id" v-model="formData.fragranceFamilyIds" class="d-none">
                      <div class="d-flex justify-content-between align-items-center">
                        <div class="fw-medium text-truncate pe-2">{{ item.name }}</div>
                        <div class="check-icon">
                          <i class="bi" :class="formData.fragranceFamilyIds.includes(item.id) ? 'bi-check-circle-fill' : 'bi-circle'"></i>
                        </div>
                      </div>
                    </label>
                  </div>
                ...
                <div v-if="formData.fragranceFamilyIds.length" class="selected-list mt-4">
                  <div class="small text-muted mb-2">Đã chọn</div>
                  <div class="d-flex flex-wrap gap-2">
                    <span v-for="item in fragranceFamilyList.filter(x => formData.fragranceFamilyIds.includes(x.id))" :key="item.id" class="badge rounded-pill bg-primary">
                      {{ item.name }}
                    </span>
                  </div>
                </div>
            ...
            <section class="content-card">
              <div class="section-header">
                ...
                <button type="button" class="btn btn-primary rounded-pill px-4" @click="addVariant">
              ...
              <div class="section-body p-0">
                <div class="variant-table m-3 mt-0">
                  <table class="table align-middle mb-0">
                    <thead>
                      <tr>
                        <th width="150">Dung tích <span class="text-danger">*</span></th>
                        <th width="180">Loại chai <span class="text-danger">*</span></th>
                        <!-- Đã bỏ cột SKU -->
                        <th width="180">Giá bán <span class="text-danger">*</span></th>
                        <th width="120">Tồn kho <span class="text-danger">*</span></th>
                        <th width="160">NSX <span class="text-danger">*</span></th>
                        <th width="160">HSD <span class="text-danger">*</span></th>
                    ...
                    <tbody>
                      <tr v-for="(variant,index) in formData.variants" :key="index">
                        <td>
                          <select v-model.number="variant.capacityId" class="form-select">
                            <option :value="0">Chọn</option>
                            <option v-for="item in capacityList" :key="item.id" :value="item.id">{{ item.value }} ml</option>
                          </select>
                        </td>
                        <td>
                          <select v-model.number="variant.bottleTypeId" class="form-select">
                            <option :value="0">Chọn</option>
                            <option v-for="item in bottleTypeList" :key="item.id" :value="item.id">{{ item.bottleTypeName ?? item.name }}</option>
                          </select>
                        </td>
                        <!-- Đã bỏ ô input SKU -->
                        <td>
                          <div class="input-group">
                            ...
                            <input v-model.number="variant.price" type="number" min="0" class="form-control text-end">
                          </div>
                        </td>
                        <td>
                          <input v-model.number="variant.stockQuantity" type="number" min="0" class="form-control text-center">
                        </td>
                        <td>
                          <input v-model="variant.manufacturingDate" type="date" class="form-control">
                        </td>
                        <td>
                          <input v-model="variant.expirationDate" type="date" class="form-control">
                        </td>
                        <td>
                          <select v-model.number="variant.status" class="form-select">
                            <option :value="1">Đang bán</option>
                            <option :value="0">Ngừng bán</option>
                          </select>
                        </td>
                        <td class="text-center">
                          <button v-if="formData.variants.length > 1" type="button" class="btn btn-outline-danger rounded-circle btn-icon mx-auto" @click="removeVariant(index)">
            ...
            <section class="content-card mb-5">
              ...
              <div class="section-body">
                <label class="upload-area">
                  <input type="file" multiple accept="image/*" hidden @change="handleImages">
                  <i class="bi bi-cloud-arrow-up"></i>
                  <h6>Kéo ảnh vào đây</h6>
                  <p>hoặc nhấn để chọn ảnh</p>
                </label>

                <!-- Thêm TransitionGroup để ảnh nhảy lên có hiệu ứng mượt -->
                <transition-group name="image-list" tag="div" class="row g-4 mt-1">
                  <div class="col-xl-3 col-lg-4 col-md-6 image-item" v-for="(img,index) in imageList" :key="img.preview">
                    <div class="image-card">
                      <div class="image-wrapper">
                        <img :src="img.preview">
                        <div class="image-overlay">
                          <button type="button" class="btn btn-light btn-sm" @click="setPrimaryImage(index)">
                      ...
                      <div class="image-footer">
                        <button v-if="img.isPrimary" type="button" class="btn btn-success w-100 rounded-pill" style="pointer-events: none; opacity: 1;">
                          Ảnh chính
                        </button>
                        <button v-else type="button" class="btn btn-outline-success w-100 rounded-pill" @click="setPrimaryImage(index)">
                          Đặt làm ảnh chính
                        </button>
                      </div>
                    </div>
...
import type {
  Product, ProductRequestDTO, ProductVariant, Brand, Category,
  Concentration, Capacity, BottleType, FragranceFamily
...
const formData = ref<ProductRequestDTO>({
  name: "", description: "", brandId: 0, categoryId: 0, concentrationId: 0,
  gender: 3, isNiche: false, status: 1, fragranceFamilyIds: [], variants: []
});
...
const addVariant = () => {
  formData.value.variants.push({
    capacityId: 0, bottleTypeId: 0, price: 0, stockQuantity: 0,
    manufacturingDate: "", expirationDate: "", status: 1
  });
};
...
const fillForm = (product: Product) => {
  ...
  // Đổ ảnh ra và sắp xếp ảnh chính lên đầu
  imageList.value = (product.images?.map(img => ({
    id: img.id,
    preview: img.imageUrl ? (img.imageUrl.startsWith("http") ? img.imageUrl : `${API_URL}${img.imageUrl}`) : "",
...
const validateForm = () => {
  if (imageList.value.length === 0) {
    Swal.fire("Thiếu ảnh", "Vui lòng chọn ít nhất 1 ảnh", "warning"); return false;
  }
  if (!formData.value.name.trim()) {
    Swal.fire("Thiếu dữ liệu", "Vui lòng nhập tên sản phẩm", "warning"); return false;
  }
  if (formData.value.brandId === 0) {
    Swal.fire("Thiếu dữ liệu", "Chưa chọn thương hiệu", "warning"); return false;
  }
  if (formData.value.categoryId === 0) {
    Swal.fire("Thiếu dữ liệu", "Chưa chọn danh mục", "warning"); return false;
  }
  if (formData.value.concentrationId === 0) {
    Swal.fire("Thiếu dữ liệu", "Chưa chọn nồng độ", "warning"); return false;
  }
  if (formData.value.fragranceFamilyIds.length === 0) {
    Swal.fire("Thiếu dữ liệu", "Vui lòng chọn ít nhất 1 nhóm hương", "warning"); return false;
  }

  for (const variant of formData.value.variants) {
    if (variant.capacityId === 0) {
      Swal.fire("Thiếu dữ liệu", "Chưa chọn dung tích cho biến thể", "warning"); return false;
    }
    if (variant.bottleTypeId === 0) {
      Swal.fire("Thiếu dữ liệu", "Chưa chọn loại chai cho biến thể", "warning"); return false;
    }
    if (variant.price <= 0) {
      Swal.fire("Thiếu dữ liệu", "Giá bán phải lớn hơn 0", "warning"); return false;
    }
    if (!variant.manufacturingDate || !variant.expirationDate) {
      Swal.fire("Thiếu dữ liệu", "Vui lòng nhập đầy đủ NSX và HSD cho các biến thể", "warning"); return false;
    }
    
    // 👇 THÊM VALIDATE NSX KHÔNG ĐƯỢC SAU HSD
    if (variant.manufacturingDate && variant.expirationDate) {
      const nsx = new Date(variant.manufacturingDate);
      const hsd = new Date(variant.expirationDate);
      if (nsx > hsd) {
        Swal.fire("Lỗi ngày tháng", "Ngày sản xuất (NSX) không được sau Hạn sử dụng (HSD)", "warning"); 
        return false;
      }
    }
  }
...
const saveData = async () => {
  ...
  try {
    ...
    const payload: ProductRequestDTO = {
      ...
      fragranceFamilyIds: formData.value.fragranceFamilyIds.map(Number)
    };
    ...
    if (isEdit.value && props.productSelected) {
      ...
      if (primaryImage?.id) {
        ...
        const updatedImages = await productService.getImagesByProduct(props.productSelected.id);
        if(updatedImages && updatedImages.length > 0) {
          await productService.setPrimaryImage(props.productSelected.id, updatedImages[updatedImages.length - 1].id);
        }
      }

      appStore.notifySuccess("Cập nhật sản phẩm thành công");
    } else {
      ...
      appStore.notifySuccess("Thêm sản phẩm thành công");
    }
...
.product-drawer {
  ...
  width: min(1550px, 96vw); height: 100%;
  background: #f8fafc;
  display: flex; flex-direction: column;
  border-radius: 24px 0 0 24px;
  overflow: hidden;
  box-shadow: -12px 0 50px rgba(15,23,42,.18);
...
.btn-close-modal {
  ...
  background: #f1f5f9; transition: .25s;
...
.content-card {
  ...
  border: 1px solid #eef2f7; position: relative; transition: .25s;
}
.content-card:hover { box-shadow: 0 18px 40px rgba(15,23,42,.07); }
.section-header {
  padding: 24px 28px;
  display: flex; justify-content: space-between; align-items: center;
...
.form-control, .form-select {
  min-height: 48px; border: 1px solid #dbe4ee; border-radius: 14px; background: #fff;
  transition: all .25s ease; font-size: 14px; color: #0f172a; padding-left: 16px;
}
.form-control:hover, .form-select:hover { border-color: #94a3b8; }
.form-control:focus, .form-select:focus { border-color: #2563eb; box-shadow: 0 0 0 4px rgba(37,99,235,.12); transform: translateY(-1px); }
...
.form-check-input { width: 50px; height: 26px; cursor: pointer; border-radius: 999px; border: 1px solid #cbd5e1; background-color: #e2e8f0; transition: .25s; }
...
/* =========================================================
   NHÓM HƯƠNG (PILL SHAPE - ĐỒNG BỘ ẢNH)
========================================================= */
.fragrance-card {
  display: block; padding: 12px 20px;
  border: 1px solid #cbd5e1; border-radius: 999px;
  cursor: pointer; transition: .25s; background: white;
  will-change: transform;
}
.fragrance-card:hover { border-color: #2563eb; transform: translateY(-2px); box-shadow: 0 8px 24px rgba(37,99,235,.08); }
.fragrance-card.active { border-color: #2563eb; background: #eff6ff; }
.check-icon { font-size: 20px; color: #2563eb; line-height: 1; }
.selected-list { border-top: 1px dashed #dbe4ee; padding-top: 20px; }

/* =========================================================
   BẢNG BIẾN THỂ (TABLE)
========================================================= */
.variant-table { border: 1px solid #e2e8f0; border-radius: 18px; overflow: auto; background: #fff; }
.variant-table table { margin: 0; min-width: 1250px; }
...
.variant-table tbody tr { transition: .2s; animation: variantFade .25s ease; }
...
.variant-table input[type="number"] { text-align: right; font-weight: 600; }
.btn-icon { width: 38px; height: 38px; padding: 0; display: flex; justify-content: center; align-items: center; border-radius: 10px; transition: .25s; }
...
.upload-area {
  ...
  border: 2px dashed #cbd5e1; border-radius: 20px; background: #fbfdff;
  display: flex; flex-direction: column; justify-content: center; align-items: center;
  cursor: pointer; transition: .25s; text-align: center; margin-bottom: 28px;
}
...
.image-card:hover { transform: translateY(-4px); box-shadow: 0 14px 35px rgba(15,23,42,.08); }
.image-wrapper { position: relative; height: 220px; overflow: hidden; background: #f8fafc; }
.image-wrapper img { width: 100%; height: 100%; object-fit: contain; transition: .35s; }
...
.drawer-footer {
  ...
  border-top: 1px solid #e2e8f0; padding: 18px 32px;
  display: flex; justify-content: flex-end; align-items: center; gap: 14px;
  position: sticky; bottom: 0; z-index: 20; box-shadow: 0 -8px 20px rgba(15,23,42,.04);
}
.drawer-footer .btn { min-width: 140px; height: 46px; font-size: 15px; font-weight: 600; border-radius: 14px; transition: .25s; }
...
@keyframes imageFade { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }

/* Reponsive */
@media (max-width: 1400px) { .product-drawer { width: 100vw; border-radius: 0; } }
@media (max-width: 992px) {
  .drawer-header { padding: 20px; height: auto; }
  .form-content { padding: 20px; }
  .drawer-footer { padding: 18px 20px; }
}
@media (max-width: 768px) {
  .drawer-title { gap: 12px; }
  .drawer-icon { width: 48px; height: 48px; font-size: 20px; }
