  ...
  >
    ...
    <div class="card shadow-sm border-0 rounded-3 mb-4">
      <div class="card-header bg-white border-bottom-0 pt-3">
        <ul class="nav nav-tabs card-header-tabs">
          <li class="nav-item" v-for="tab in tabs" :key="tab.value">
            <a
              class="nav-link cursor-pointer fw-medium"
      ...
      <div class="card-body bg-white border-top">
        <div class="row g-3">
          <div class="col-md-5">
            <div class="input-group">
              ...
              <input
                ...
                type="text"
                class="form-control"
                placeholder="Tìm tên KH, email, tên SP, SKU, nội dung..."
              />
            </div>
          </div>
          <div class="col-md-2">
          ...
          <div class="col-md-2">
            <button @click="fetchReviews" class="btn btn-primary w-100">
              Lọc dữ liệu
            </button>
          </div>
        </div>
          ...
          <tbody>
            <tr v-for="item in reviews" :key="item.id">
              ...
              <td>
                <span
                  class="badge"
                  :class="getStatusBadgeClass(item.approvalStatus)"
                >
              ...
              <td class="text-center">
                ...
                <template
                  v-if="
                    ...
                    (authStore.isManager || authStore.isOwner)
                  "
                >
                  <button
                ...
                <template
                  v-if="
                    (item.approvalStatus === 2 || item.approvalStatus === 3) &&
                    (authStore.isManager || authStore.isOwner)
                  "
                >
                  <button
                ...
                <span
                  v-if="!authStore.isManager && !authStore.isOwner"
    ...
    >
      ...
      <button
        ...
        class="btn btn-outline-light position-absolute border-0"
        style="
          top: 20px;
          right: 20px;
          z-index: 1060;
      ...
      <div
        ...
        style="height: 100vh; margin: 0 auto; max-width: 900px"
        @click.self="showMediaModal = false"
      >
        <div
          class="modal-content bg-transparent border-0 w-100 d-flex flex-column justify-content-center h-100"
        >
          <!-- KHUNG HIỂN THỊ CHÍNH -->
          <div
            class="position-relative d-flex justify-content-center align-items-center w-100"
            style="flex: 1; max-height: 75vh"
          >
            <!-- Nút Trái -->
            <button
              v-if="activeMediaIndex > 0"
              @click="prevMedia"
              class="btn btn-light position-absolute start-0 translate-middle-y"
              style="
                top: 50%;
                border-radius: 50%;
                padding: 10px 15px;
            ...
            <template v-if="currentMedia[activeMediaIndex]">
              <img
                v-if="
                  currentMedia[activeMediaIndex].mediaType === 'image' ||
                  currentMedia[activeMediaIndex].mediaType === 'IMAGE'
                "
              ...
              <video
                ...
                autoplay
                class="img-fluid rounded shadow"
                style="max-height: 75vh; outline: none"
              ></video>
            </template>

            <!-- Nút Phải -->
            <button
              v-if="activeMediaIndex < currentMedia.length - 1"
              @click="nextMedia"
              class="btn btn-light position-absolute end-0 translate-middle-y"
              style="
                top: 50%;
                border-radius: 50%;
                padding: 10px 15px;
          ...
          >
            ...
            >
              <img
                v-if="m.mediaType === 'image' || m.mediaType === 'IMAGE'"
                :src="m.mediaUrl"
                class="w-100 h-100"
                style="object-fit: cover"
              />
              ...
              <div
                v-else
                class="w-100 h-100 bg-dark position-relative d-flex align-items-center justify-content-center"
              >
                <video
                  :src="m.mediaUrl"
                ...
                ></video>
                <i
                  class="bi bi-play-circle-fill text-white position-relative z-1 fs-4"
                ></i>
              </div>
            </div>
          </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <textarea
              ...
              class="form-control"
              rows="3"
              placeholder="Ví dụ: Hình ảnh/video chứa nội dung không hợp lệ..."
            ></textarea>
          </div>
          <div class="modal-footer">
            <button @click="showRejectModal = false" class="btn btn-light">
              Hủy
            </button>
            <button
              @click="submitReject"
...
import type {
  Review,
  ReviewMedia,
} from "@/modules/admin/feature/review/types/review.type";
...
const fetchReviews = async () => {
  try {
    const params = {
      ...
      hasMedia:
        filters.value.hasMedia !== "" ? filters.value.hasMedia : undefined,
      page: 0,
      size: 50,
    };
