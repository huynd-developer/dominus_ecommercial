<template>
  ...
  >
    ...
    <div
      class="py-1 px-2 d-flex align-items-center justify-content-center border-bottom border-slate-100 flex-shrink-0 overflow-hidden"
      style="height: 75px"
    >
      <img
        src="@/assets/logo.png"
          ...
          <div v-else class="d-flex flex-column">
            ...
            >
              <div class="d-flex align-items-center gap-3">
                <i
                  class="fs-5 nav-icon-width"
                  :class="[
                    item.icon,
                    { 'text-dark-peel': isGroupActive(item) },
                  ]"
                ></i>
                <span>{{ item.name }}</span>
              </div>
              <i
                class="bi bi-chevron-down transition-transform arrow-icon"
                :class="{ 'rotate-180': openMenus[item.id] }"
              ></i>
            </div>

            <div
              v-if="openMenus[item.id]"
              class="d-flex flex-column gap-1 mt-1 ps-4 submenu-wrapper animate__animated animate__fadeIn animate__faster"
            >
              <router-link
    ...
    >
      ...
      >
        <div
          class="bg-dark-peel text-white rounded-circle fw-bold d-flex align-items-center justify-content-center shadow-peel avatar-box flex-shrink-0"
        >
          {{ userAvatar }}
        </div>
        <div class="overflow-hidden flex-grow-1 text-nowrap text-truncate pe-1">
          <h6
            class="mb-0 fw-bold text-slate-800 text-truncate"
