          ...
          >
            ...
            <td class="text-center">
              ...
              <span
                ...
                :class="
                    getProductSellableQuantity(product) > 0
                      ? 'stock-ok'
                      : 'stock-empty'
                  "
              ...
              >
                ...
                {{ getProductSellableQuantity(product) }}
...
const displayProducts = computed(() => {
  ...
  return posStore.filteredProducts.filter((product) => {
    ...
    const stock = getProductSellableQuantity(product);

    /*
     * POS chỉ hiển thị SKU còn hàng CÓ THỂ BÁN.
     * Nguồn số lượng là sellableQuantity từ InventoryLot.
     */
...
const isExpired = (product: any) => {
  /*
   * Không tự tính HSD bằng field ngày compatibility.
   * Backend xác định expired/sellable từ InventoryLot.
   */
  return Boolean(product?.expired);
};
...
const getDisabledReason = (product: any) => {
  ...
  /*
   * Không tự suy luận NSX/HSD tại FE.
   * sellable/unavailableReason từ BE là nguồn quyết định.
   */
  return "";
};
