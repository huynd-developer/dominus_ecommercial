<template>
  <div class="cart-left">
    ...
    <div v-else class="item-list">
      <div v-for="item in cartItems" :key="item.cartItemId" :class="['cart-item', { 'item-unavailable': !isItemAvailable(item) }]">
        <img :src="getItemImage(item)" class="item-img" :alt="item.productName || 'Sản phẩm'" @error="handleImageError" />
        ...
        <div class="item-info">
          ...
          <div class="variant-grid">
            <template v-if="getVariantsList(item).length > 0">
              ...
              >
                ...
                >
                  {{ formatVariantLabel(v) }} ({{ formatCurrency(v.price || v.salePrice || v.originalPrice) }})
                </option>
              </select>
            </template>
            <template v-else>
          ...
          <div class="date-grid">
            <span>NSX: <strong>{{ formatDate(item.manufacturingDate) }}</strong></span>
            <span :class="{ 'text-danger': isExpired(item) }">HSD: <strong>{{ formatDate(item.expirationDate) }}</strong></span>
          </div>

          <p class="stock-line">Tồn kho: <strong>{{ Number(item.stockQuantity || 0) }}</strong></p>

          <div class="status-row">
            <span :class="['status-badge', isItemAvailable(item) ? 'status-ok' : 'status-error']">
              {{ isItemAvailable(item) ? "Có thể mua" : "Không khả dụng" }}
            </span>
          </div>

          <p v-if="!isItemAvailable(item)" class="unavailable-text">{{ getUnavailableReason(item) }}</p>

          <div class="qty-wrapper">
            <button type="button" @click="changeQuantity(item, -1)" :disabled="Number(item.quantity || 0) <= 1">−</button>
            <input type="text" :value="Number(item.quantity || 0)" readonly />
            <button type="button" @click="changeQuantity(item, 1)" :disabled="!isItemAvailable(item) || Number(item.quantity || 0) >= Number(item.stockQuantity || 0)">+</button>
          </div>
        </div>
        ...
        <div class="item-action">
          ...
          <template v-else>
            <span class="unit-price">Đơn giá: {{ formatCurrency(item.price) }}</span>
            <span class="price">{{ formatCurrency(getLineTotal(item)) }}</span>
          </template>
...
const props = defineProps<{ cartItems: CartItem[]; isLoading: boolean; isUpdating?: boolean; }>();
...
const changeQuantity = (item: CartItem, delta: number) => {
  ...
  updateTimeout = setTimeout(() => { emit("update-qty", item, newQty); }, 400);
};
...
const getVariantsList = (item: CartItem) => {
  ...
  const possibleProducts = [
    ...
    i?.variant?.product, i?.Variant?.Product,
    i 
  ];
  ...
  for (const p of possibleProducts) {
    ...
    const lists = [
      ...
      p.productVariantList, p.ProductVariantList,
      p.productVariantResponses, p.ProductVariantResponses,
      p.productVariantDTOs, p.ProductVariantDTOs,
      p.lstProductVariant, p.LstProductVariant,
      p.items, p.Items
    ];
...
const handleImageError = (event: Event) => {
  ...
  if (target) { target.onerror = null; target.src = FALLBACK_IMAGE; }
};
...
.loading { padding: 30px 0; color: #718096; }
...
.item-unavailable { opacity: 0.72; }
.item-img { width: 140px; height: 140px; object-fit: cover; border-radius: 12px; border: 1px solid #eaeaea; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.04); background: #f8fafc; }
...
.variant-select:hover:not(:disabled) { border-color: #b78d52; background-color: #fcf4e8; }
.variant-select:disabled { opacity: 0.6; cursor: not-allowed; background-color: #f8fafc; border-color: #e2e8f0; color: #94a3b8; background-image: none; }

.date-grid { display: flex; flex-wrap: wrap; gap: 12px; font-size: 13px; color: #64748b; }
.date-grid strong { color: #06132b; }
.text-danger strong, .text-danger { color: #dc2626 !important; }
.stock-line { margin: 0; color: #64748b; font-size: 13px; }
.stock-line strong { color: #06132b; }
.status-row { display: flex; align-items: center; gap: 8px; }
.status-badge { width: fit-content; border-radius: 999px; padding: 4px 10px; font-size: 12px; font-weight: 800; }
.status-ok { background: #dcfce7; color: #166534; border: 1px solid #bbf7d0; }
.status-error { background: #fee2e2; color: #991b1b; border: 1px solid #fecaca; }
.unavailable-text { margin: 0; color: #dc2626; background: #fee2e2; border: 1px solid #fecaca; padding: 8px 10px; border-radius: 8px; width: fit-content; font-size: 13px; }
...
.qty-wrapper button:hover:not(:disabled) { background: #e2e8f0; }
...
@media (max-width: 768px) {
  .cart-left { padding: 24px; }
  .cart-item { align-items: flex-start; gap: 16px; }
  .item-img { width: 90px; height: 90px; }
  .item-action { min-height: auto; }
