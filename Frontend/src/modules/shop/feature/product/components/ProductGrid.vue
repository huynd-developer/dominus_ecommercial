  ...
  <div class="product-grid-wrapper">
      ...
      >
        ...
        <div class="card-info">
          ...
          <div class="card-rating">
            ...
            <span class="score"
              >{{ getRatingScore(item) }} | {{ getReviewCount(item) }} đánh
              giá</span
            >
          </div>
          <div class="card-price-box">
            ...
            <span
              ...
              >{{ formatCurrency(getOldPrice(item)) }}</span
            >
          </div>
          <div class="card-actions">
            <button
              ...
              @click.stop="openVariantModal('BUY', item)"
            >
              Mua ngay
            </button>
            <button
          ...
          <div v-if="getVariantStock(item) <= 0" class="card-stock-warning">
            Tạm hết hàng
          </div>
        </div>
      </div>
    ...
    <div class="compare-bar" :class="{ show: compareList.length > 0 }">
      <div class="cb-container">
        <div class="cb-left">
          ...
          <div class="cb-slots">
            ...
            >
              ...
              <button class="btn-remove-cb" @click="removeFromCompare(p)">
                ✕
              </button>
            </div>
            <div
              class="cb-slot empty-slot cursor-pointer"
              v-for="i in Math.max(0, 3 - compareList.length)"
              :key="'empty' + i"
        ...
        <div class="cb-right">
          <button class="cb-btn-clear" @click="compareList = []">
            Xóa hết
          </button>
          <button
      ...
      >
        <div class="compare-modal-box">
          <div class="cm-header">
            ...
            <button class="cm-close" @click="showCompareModal = false">
              ✕
            </button>
          </div>
          <div class="cm-body">
...
const getCurrentRole = () =>
  String(localStorage.getItem("role") || localStorage.getItem("userRole") || "")
    .replace("ROLE_", "")
    .toUpperCase()
    .trim();
  ...
  if (isInCompare(item)) {
    ...
    if (compareList.value.length >= MAX_COMPARE) {
      showToast(
        "warning",
        "Giới hạn",
        `Chỉ được so sánh tối đa ${MAX_COMPARE} sản phẩm!`
      );
      return;
    }
  ...
  if (val) {
    compareList.value.forEach((p) => {
      ...
      if (!compareVariantIds.value[id]) {
        ...
        compareVariantIds.value[id] = Number(
...
const getCompareVariant = (p: any) => {
  ...
  if (vId && p.variants && Array.isArray(p.variants)) {
    const found = p.variants.find(
      (v: any) => (v.productVariantId || v.variantId || v.id) === vId
    );
...
const getCompareOriginalPrice = (p: any) => {
  ...
  let vOrig = getSafeNumber(
    v?.originalPrice ?? v?.oldPrice ?? v?.listPrice ?? v?.price ?? v?.Price
  );
  if (vOrig > sale) return vOrig;
  let pOrig = getSafeNumber(
    p?.originalPrice ??
      p?.oldPrice ??
      p?.listPrice ??
      p?.price ??
...
const getGenderText = (item: any) => {
  const g = item?.gender;
  if (g === 1 || String(g) === "1" || String(g).toLowerCase() === "nam")
    return "Nam";
  if (g === 2 || String(g) === "2" || String(g).toLowerCase() === "nữ")
    return "Nữ";
  if (g === 0 || String(g) === "0" || String(g).toLowerCase() === "unisex")
    return "Unisex";
  return typeof g === "object"
    ? g?.name || "Đang cập nhật"
    : g || "Đang cập nhật";
};
...
const getOccasionText = (item: any) => {
  ...
  if (Array.isArray(item?.occasions) && item.occasions.length > 0) {
    occasions = item.occasions.map((i: any) =>
  ...
  if (Array.isArray(item?.seasons) && item.seasons.length > 0) {
    seasons = item.seasons.map((i: any) =>
      typeof i === "object" ? i?.name : i
    );
  }
  ...
  if (occasions.length > 0 || seasons.length > 0) {
    ...
    if (seasons.length > 0)
      result.push(`Mùa: ${seasons.filter(Boolean).join(", ")}`);
    return result.join(" | ");
  }
  ...
  const con = String(
    getAttributeText(item, "concentration") || ""
  ).toLowerCase();

  if (
    scent.includes("wood") ||
    scent.includes("gỗ") ||
    scent.includes("oriental") ||
    scent.includes("phương đông") ||
    scent.includes("gourmand")
  ) {
    return "Đi tiệc, Hẹn hò, Sự kiện quan trọng | Hợp Thu - Đông";
  }
  if (
    scent.includes("citrus") ||
    scent.includes("cam chanh") ||
    scent.includes("aquatic") ||
    scent.includes("nước") ||
    con.includes("cologne") ||
    con.includes("edt")
  ) {
    return "Đi học, Đi làm (Office), Thể thao, Dạo phố | Hợp Xuân - Hè";
  }
  if (
    scent.includes("floral") ||
    scent.includes("hoa") ||
    scent.includes("fruity") ||
    scent.includes("trái cây")
  ) {
...
const getStyleText = (item: any) => {
  ...
  if (scent.includes("wood") || scent.includes("gỗ"))
    return "Sang trọng, Trưởng thành, Ấm áp";
  if (scent.includes("floral") || scent.includes("hoa"))
    return "Nữ tính, Thanh lịch, Quyến rũ";
  if (scent.includes("citrus") || scent.includes("cam chanh"))
    return "Năng động, Tươi mát, Trẻ trung";
  if (scent.includes("oriental") || scent.includes("phương đông"))
    return "Gợi cảm, Bí ẩn, Cuốn hút";
  if (scent.includes("fruity") || scent.includes("trái cây"))
    return "Ngọt ngào, Đáng yêu, Tươi mới";
  if (scent.includes("aquatic") || scent.includes("nước"))
    return "Phóng khoáng, Mát mẻ, Thể thao";
...
const isBestValue = (p: any) => {
  const validValues = compareList.value
...
const getRatingValue = (item: any) => {
  const id = Number(item?.id || item?.productId || 0);
  let raw =
    id > 0 && syncedRatingsMap.value[id] !== undefined
      ? syncedRatingsMap.value[id]
      : Number(item?.averageRating || item?.avgRating || item?.rating || 0);
...
const syncGridRatings = () => {
  ...
  props.productList.forEach(async (item) => {
    ...
    const currentCount = Number(
      item?.reviewCount || item?.reviews || item?.totalReviews || 0
    );
    if (
      id > 0 &&
      currentCount === 0 &&
      syncedReviewsMap.value[id] === undefined
    ) {
      try {
        const res = await api.get(`/v1/products/${id}`);
        const data = res.data?.data || res.data;
        if (data) {
          syncedRatingsMap.value[id] = Number(
            data.averageRating || data.avgRating || data.rating || 0
          );
          syncedReviewsMap.value[id] = Number(
  ...
  for (const arr of imageArrays) {
    if (Array.isArray(arr)) {
      ...
      if (primaryObj) {
        appendImage(
          images,
          primaryObj?.imageUrl || primaryObj?.url || primaryObj
        );
      }
    }
  }
...
const getProductImage = (item: any) =>
  getProductImages(item)[0] || getPlaceholderImage();
...
const getVariantStock = (item: any) =>
  getSellableQuantity(getPrimaryVariantObject(item));
const getVariantPrice = (item: any) => {
  ...
  if (variant?.price != null || variant?.Price != null)
    return getSafeNumber(variant?.price ?? variant?.Price);
  return Number(
    item?.salePrice ?? item?.flashSalePrice ?? item?.price ?? item?.Price ?? 0
  );
};

const getOldPrice = (item: any) => {
  const variant = getPrimaryVariantObject(item);
  if (variant?.originalPrice != null || variant?.oldPrice != null)
    return Number(variant.originalPrice || variant.oldPrice);
...
const setFavorited = (variantId: number, value: boolean) =>
  (favoritedMap.value = { ...favoritedMap.value, [variantId]: value });
...
const modalImage = computed(() => {
  ...
  return modalProduct.value
    ? getProductImage(modalProduct.value)
    : getPlaceholderImage();
});
...
const validateQuantity = () => {
  ...
  if (Number.isNaN(val) || val < 1) quantity.value = 1;
    ...
    showToast("warning", "Hết hàng", `Chỉ còn ${maxQuantity.value} trong kho!`);
  } else quantity.value = Math.floor(val);
};

const increaseQuantity = () => {
  if (quantity.value >= 10) {
    showToast("warning", "Giới hạn mua", "Chỉ được mua tối đa 10 sản phẩm!");
    return;
  }
  ...
  try {
    ...
    if (item?.variants && Array.isArray(item.variants)) {
      item.variants.forEach((pv: any) => {
        ...
        if (vId) {
          if (pv.salePrice != null || pv.flashSalePrice != null)
            flashSalePriceMap.set(
              vId,
              Number(pv.salePrice || pv.flashSalePrice)
            );
          if (
            pv.originalPrice != null ||
            pv.oldPrice != null ||
            pv.price != null
          )
            originalPriceMap.set(
              vId,
              Number(pv.originalPrice || pv.oldPrice || pv.price)
            );
        }
      });
    }
    ...
    const processedVariants = rawVariants.map((v: any) => {
      ...
      const mappedOriginalPrice =
        originalPriceMap.get(vId) ?? v.originalPrice ?? v.oldPrice ?? v.price;
...
const confirmAction = async (type: "CART" | "BUY") => {
  ...
  try {
    ...
    showToast(
      "error",
      "Lỗi",
      error?.response?.data?.message || "Không thể thực hiện."
    );
  } finally {
...
.sale-badge ~ .expiry-warning-badge {
  top: 48px;
}
...
.qty-keyword input[type="number"]::-webkit-outer-spin-button {
